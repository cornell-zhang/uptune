...
def connect(dbstr):
  engine = create_engine(dbstr, echo = False, connect_args={'check_same_thread': False})
  ...
  if engine.dialect.has_table(connection, "program"):
    ...
    if not engine.dialect.has_table(connection, "_meta"):
      ...
      connection.close()
      raise Exception("Your uptune.opentuner database is currently out of date. Save a back up and reinitialize")
  ...
  if engine.dialect.has_table(connection, "_meta"):
    ...
    if not DB_VERSION == version:
      raise Exception('Your uptune.opentuner database version {} is out of date with the current version {}'.format(version, DB_VERSION))

...
from uptune.opentuner.resultsdb.models import Result, TuningRun
from uptune.opentuner.search import manipulator
...
argparser = argparse.ArgumentParser(parents=uptune.argparsers())
...
class GccFlagsTuner(uptune.opentuner.measurement.MeasurementInterface):
...
# ---------------------------------------------------
@ray.remote
class ActorProcess(RunProgram):
    """
    Ray Actor to be called by ParallelTuning 
    """
    def init(self):
      # -----------------------
      # temp for gcc test
      ...
      self.compile_results = {'ok': 0, 'timeout': 1, 'error': 2}
    ...
    def run_with_flags(self, flags, limit):
      """ compile and pass to run_precompiled"""
      return self.run_precompiled(None, limit,
    ...
    def run_precompiled(self, desired_result, limit, compile_result,
                        result_id):
    ...
    def run(self, dr):
      """ run the compile_and_run""" 
      self.init()
      self.start_run()

      cfg = dr.configuration.data
      limit = self.args.runtime_limit
      compile_result = self.compile(cfg, 0)
      result = self.run_precompiled(dr, limit, compile_result, 0)
...
if __name__ == '__main__':
    ...
    pt = ProgramTune(ActorProcess, args)
    pt.main()

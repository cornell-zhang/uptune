               ...
               velocity_t outputs[MAX_HEIGHT * MAX_WIDTH])
{
  ...
  FLOW_OUTER: for(int r=0; r<MAX_HEIGHT; r++)
  {
    FLOW_INNER: for(int c=0; c<MAX_WIDTH; c++)
    {
      ...
      outputs[r * MAX_WIDTH + c].x = (vel_pixel_t)buf[0];
      outputs[r * MAX_WIDTH + c].y = (vel_pixel_t)buf[1];
...
extern "C"
{
                    ...
                    velocity_t outputs[MAX_HEIGHT * MAX_WIDTH])
  {
    ...
    #pragma HLS INTERFACE m_axi port=frames offset=slave bundle=gmem0
    #pragma HLS INTERFACE m_axi port=outputs offset=slave bundle=gmem1
